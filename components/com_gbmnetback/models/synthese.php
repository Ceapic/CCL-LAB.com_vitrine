...
class Synthese  {
	function AfficheAnalyseEchantillonMATERIAU($echantillon, $type_dossier) {
		...
		$query = "Select *
				from `" . $config->get('GBMNet_db') . "`.analyse_materiau, `" . $config->get('db') . "`." . $config->get('dbprefix') . "users
				...
				and analyse_materiau.valideur_analyse_materiau = " . $config->get('dbprefix') . "users.id
	...
	function AfficheAnalyseEchantillonMEST($echantillon, $type_dossier) {
		...
		$db = &GBMNet::getDBOGBMNet();
		$query = "Select *
				from `" . $config->get('GBMNet_db') . "`.analyse_mest, `" . $config->get('db') . "`." . $config->get('dbprefix') . "users
				where echantillon_analyse_mest = " . $echantillon . "
				and type_dossier_analyse_mest = " . $type_dossier . "
				and validation_analyse_mest = 1
				and analyse_mest.valideur_analyse_mest = " . $config->get('dbprefix') . "users.id
	...
	function AfficheAnalyseEchantillonMETA1($echantillon, $type_dossier) {
		...
		$db = &GBMNet::getDBOGBMNet();
		$query = "Select *
				from `" . $config->get('GBMNet_db') . "`.analyse_meta_1, `" . $config->get('db') . "`." . $config->get('dbprefix') . "users
				where echantillon_analyse_meta_1 = " . $echantillon . "
				and type_dossier_analyse_meta_1 = " . $type_dossier . "
				and validation_analyse_meta_1 = 1
				and analyse_meta_1.valideur_analyse_meta_1 = " . $config->get('dbprefix') . "users.id
	...
	function AfficheAnalyseEchantillonMETAv0($echantillon, $type_dossier) {
		...
		$db = &GBMNet::getDBOGBMNet();
		$query = "Select *
				from `" . $config->get('GBMNet_db') . "`.analyse_meta_v0, `" . $config->get('db') . "`." . $config->get('dbprefix') . "users
				...
				and analyse_meta_v0.valideur_analyse_meta_v0 = " . $config->get('dbprefix') . "users.id
	...
	function getAnalysesEchantillonMETAv1($echantillon, $type_dossier) {
		...
		$query = "SELECT 
				...
				JOIN {$config->get('db')}." . $config->get('dbprefix') . "users ON " . $config->get('dbprefix') . "users.id = analyse_meta_v1.id_valideur
				WHERE 
					analyse_meta_v1.id_echantillon = {$echantillon}
					AND analyse_meta_v1.type_dossier = {$type_dossier}
					AND analyse_meta_v1.validation = 1
					AND analyse_meta_v1.id_valideur = " . $config->get('dbprefix') . "users.id
				ORDER BY 
	...
	function AfficheRapportEchantillonACR($id_echantillon) {
		...
		$db = &GBMNet::getDBOGBMNet();
		$query = "Select *
				From `" . $config->get('GBMNet_db') . "`.rapport_acr, `" . $config->get('db') . "`." . $config->get('dbprefix') . "users
				Where echantillon_rapport_acr = " . $id_echantillon . "
				and validation_rapport_acr = 1
				and rapport_acr.valideur_rapport_acr = " . $config->get('dbprefix') . "users.id
	...
	function AfficheRapportEchantillonMATERIAU($id_echantillon) {
		...
		$db = &GBMNet::getDBOGBMNet();
		$query = "Select *
				From `" . $config->get('GBMNet_db') . "`.rapport_materiau, `" . $config->get('db') . "`." . $config->get('dbprefix') . "users
				Where echantillon_rapport_materiau = " . $id_echantillon . "
				and validation_rapport_materiau = 1
				and rapport_materiau.valideur_rapport_materiau = " . $config->get('dbprefix') . "users.id
	...
	function AfficheRapportEchantillonMEST($id_echantillon) {
		...
		$db = &GBMNet::getDBOGBMNet();
		$query = "Select *
				From `" . $config->get('GBMNet_db') . "`.rapport_mest, `" . $config->get('db') . "`." . $config->get('dbprefix') . "users
				Where echantillon_rapport_mest = " . $id_echantillon . "
				and validation_rapport_mest = 1
				and rapport_mest.valideur_rapport_mest = " . $config->get('dbprefix') . "users.id
	...
	function AfficheRapportEchantillonMETA11($id_echantillon) {
		...
		$db = &GBMNet::getDBOGBMNet();
		$query = "Select *
				From `" . $config->get('GBMNet_db') . "`.rapport_meta_1, `" . $config->get('db') . "`." . $config->get('dbprefix') . "users
				Where echantillon_rapport_meta_1 = " . $id_echantillon . "
				and validation_rapport_meta_1 = 1
				and rapport_meta_1.valideur_rapport_meta_1 = " . $config->get('dbprefix') . "users.id
	...
	function AfficheRapportEchantillonMETA050($id_echantillon) {
		...
		$db = &GBMNet::getDBOGBMNet();
		$query = "Select *
				From `" . $config->get('GBMNet_db') . "`.rapport_meta_050, `" . $config->get('db') . "`." . $config->get('dbprefix') . "users
				Where echantillon_rapport_meta_050 = " . $id_echantillon . "
				and validation_rapport_meta_050 = 1
				and rapport_meta_050.valideur_rapport_meta_050 = " . $config->get('dbprefix') . "users.id
	...
	function AfficheRapportEchantillonMETA12($id_echantillon) {
		...
		$db = &GBMNet::getDBOGBMNet();
		$query = "Select *
				From `" . $config->get('GBMNet_db') . "`.rapport_meta_1_2, `" . $config->get('db') . "`." . $config->get('dbprefix') . "users
				Where echantillon_rapport_meta_1_2 = " . $id_echantillon . "
				and validation_rapport_meta_1_2 = 1
				and rapport_meta_1_2.valideur_rapport_meta_1_2 = " . $config->get('dbprefix') . "users.id
	...
	function AfficheRapportEchantillonMETA269($id_echantillon) {
		...
		$db = &GBMNet::getDBOGBMNet();
		$query = "Select *
				From `" . $config->get('GBMNet_db') . "`.rapport_meta_269, `" . $config->get('db') . "`." . $config->get('dbprefix') . "users
				Where echantillon_rapport_meta_269 = " . $id_echantillon . "
				and validation_rapport_meta_269 = 1
				and rapport_meta_269.valideur_rapport_meta_269 = " . $config->get('dbprefix') . "users.id
	...
	function AfficheRapportEchantillonMETA269v2($id_echantillon) {
		...
		$db = &GBMNet::getDBOGBMNet();
		$query = "Select *
				From `" . $config->get('GBMNet_db') . "`.rapport_meta_269_v2, `" . $config->get('db') . "`." . $config->get('dbprefix') . "users
				Where echantillon_rapport_meta_269_v2 = " . $id_echantillon . "
				and validation_rapport_meta_269_v2 = 1
				and rapport_meta_269_v2.valideur_rapport_meta_269_v2 = " . $config->get('dbprefix') . "users.id
	...
	function AfficheRapportEchantillonMETA269v3($id_echantillon) {
		...
		$db = &GBMNet::getDBOGBMNet();
		$query = "Select *
				From `" . $config->get('GBMNet_db') . "`.rapport_meta_269_v3, `" . $config->get('db') . "`." . $config->get('dbprefix') . "users
				Where echantillon_rapport_meta_269_v3 = " . $id_echantillon . "
				and validation_rapport_meta_269_v3 = 1
				and rapport_meta_269_v3.valideur_rapport_meta_269_v3 = " . $config->get('dbprefix') . "users.id
	...
	function AfficheRapportEchantillonLingettePlomb($id_echantillon) {
		...
		$db = &GBMNet::getDBOGBMNet();
		$query = "Select *
				From `" . $config->get('GBMNet_db') . "`.rapport_lingette_plomb, `" . $config->get('db') . "`." . $config->get('dbprefix') . "users
				Where echantillon_rapport_lingette_plomb = " . $id_echantillon . "
				and validation_rapport_lingette_plomb = 1
				and rapport_lingette_plomb.valideur_rapport_lingette_plomb = " . $config->get('dbprefix') . "users.id
	...
	function AfficheRapportEchantillonLixiviation($id_echantillon) {
		...
		$db = &GBMNet::getDBOGBMNet();
		$query = "Select *
				From `" . $config->get('GBMNet_db') . "`.rapport_lixiviation, `" . $config->get('db') . "`." . $config->get('dbprefix') . "users
				Where echantillon_rapport_lixiviation = " . $id_echantillon . "
				and validation_rapport_lixiviation = 1
				and rapport_lixiviation.valideur_rapport_lixiviation = " . $config->get('dbprefix') . "users.id
	...
	function AfficheRapportEchantillonEcaillePeinture($id_echantillon) {
		...
		$db = &GBMNet::getDBOGBMNet();
		$query = "Select *
				From `" . $config->get('GBMNet_db') . "`.rapport_ecaille_peinture, `" . $config->get('db') . "`." . $config->get('dbprefix') . "users
				Where echantillon_rapport_ecaille_peinture = " . $id_echantillon . "
				and validation_rapport_ecaille_peinture = 1
				and rapport_ecaille_peinture.valideur_rapport_ecaille_peinture = " . $config->get('dbprefix') . "users.id
	...
	function AfficheRapportEchantillonAirPlomb($id_echantillon) {
		...
		$db = &GBMNet::getDBOGBMNet();
		$query = "Select *
				From `" . $config->get('GBMNet_db') . "`.rapport_air_plomb, `" . $config->get('db') . "`." . $config->get('dbprefix') . "users
				Where echantillon_rapport_air_plomb = " . $id_echantillon . "
				and validation_rapport_air_plomb = 1
				and rapport_air_plomb.valideur_rapport_air_plomb = " . $config->get('dbprefix') . "users.id
	...
	function AfficheRapportEchantillonAirPlombv2($id_echantillon) {
		...
		$db = &GBMNet::getDBOGBMNet();
		$query = "Select *
				From `" . $config->get('GBMNet_db') . "`.rapport_air_plomb_v2, `" . $config->get('db') . "`." . $config->get('dbprefix') . "users
				Where echantillon_rapport_air_plomb_v2 = " . $id_echantillon . "
				and validation_rapport_air_plomb_v2 = 1
				and rapport_air_plomb_v2.valideur_rapport_air_plomb_v2 = " . $config->get('dbprefix') . "users.id
	...
	function AfficheRapportEchantillonMETA31($id_echantillon) {
		...
		$db = &GBMNet::getDBOGBMNet();
		$query = "Select *
				From `" . $config->get('GBMNet_db') . "`.rapport_meta_3, `" . $config->get('db') . "`." . $config->get('dbprefix') . "users
				Where echantillon_rapport_meta_3 = " . $id_echantillon . "
				and validation_rapport_meta_3 = 1
				and rapport_meta_3.valideur_rapport_meta_3 = " . $config->get('dbprefix') . "users.id
	...
	function AfficheRapportEchantillonMETA32($id_echantillon) {
		...
		$db = &GBMNet::getDBOGBMNet();
		$query = "Select *
				From `" . $config->get('GBMNet_db') . "`.rapport_meta_3_2, `" . $config->get('db') . "`." . $config->get('dbprefix') . "users
				Where echantillon_rapport_meta_3_2 = " . $id_echantillon . "
				and validation_rapport_meta_3_2 = 1
				and rapport_meta_3_2.valideur_rapport_meta_3_2 = " . $config->get('dbprefix') . "users.id
	...
	function ListeAnalyseMateriauDate($debut, $fin) {
		...
		$db = &GBMNet::getDBOGBMNet();
		$query = "Select *
				from `" . $config->get('GBMNet_db') . "`.echantillon_analyse, `" . $config->get('GBMNet_db') . "`.dossier, `" . $config->get('GBMNet_db') . "`.client_analyse, `" . $config->get('GBMNet_db') . "`.analyse_materiau, `" . $config->get('db') . "`." . $config->get('dbprefix') . "users
				where fiche_analyse_echantillon_analyse = 1
				and dossier_echantillon_analyse = id_dossier
				...
				and client_dossier = id_client_analyse
				and valideur_analyse_materiau = " . $config->get('dbprefix') . "users.id";
		$db->setQuery($query);
	...
	function ListeAnalyseMETADate($debut, $fin) {
		...
		$db = &GBMNet::getDBOGBMNet();
		$query = "Select *
				from `" . $config->get('GBMNet_db') . "`.echantillon_analyse, `" . $config->get('GBMNet_db') . "`.dossier, `" . $config->get('GBMNet_db') . "`.client_analyse, `" . $config->get('GBMNet_db') . "`.analyse_meta_1, `" . $config->get('GBMNet_db') . "`.type_prelevement, `" . $config->get('db') . "`." . $config->get('dbprefix') . "users
				where fiche_analyse_echantillon_analyse = 1
				and dossier_echantillon_analyse = id_dossier
				...
				and client_dossier = id_client_analyse
				and valideur_analyse_meta_1 = " . $config->get('dbprefix') . "users.id";
		$db->setQuery($query);
