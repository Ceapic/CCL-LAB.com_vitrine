...
class gbmnetBackModel {
	...
	function RapportFinalClientMission($id_client, $id_mission) {
		$db = &GBMNet::getDBOGBMNet();
		$query = "SELECT id, revision, path
				FROM 
					mission
				JOIN rapport_final_mission rfm ON rfm.id_mission = mission.id_mission
				WHERE 
					rfm.id_mission = {$id_mission}
					AND client_mission = {$id_client}
	...
	// ======================================================================================================
	// Liste échantillon analyse
	// ======================================================================================================

	function ListeEchantillonAnalyse($id_client) {
		$db = &GBMNet::getDBOGBMNet();
		$query = "SELECT 
					id_dossier, 
					id_echantillon_analyse, 
					chantier_analyse_dossier, 
					ref_client_echantillon_analyse, 
					ref_dossier, 
					ref_echantillon_analyse
				FROM 
					dossier, 
				INNER JOIN echantillon_analyse ON echantillon_analyse.dossier_echantillon_analyse = dossier.id_dossier
				WHERE 
					client_dossier = " . $id_client . "
					AND type_dossier = 2					
				ORDER BY 
					dossier.id_dossier DESC, 
	...
	function ListeEchantillonAnalyse2($id_client, $date_debut, $date_fin) {
		$db = &GBMNet::getDBOGBMNet();
		$typeAnalyse = CEAPICWorldModelRapport::getTypeRapportByType(CEAPICWorldModelRapport::TYPE_RAPPORT_ANALYSE);
		...
		foreach ($typeAnalyse as $oneTypeAnalyse) {
			...
			$queryArray[] = "SELECT 
							...
							{$validationColumn} AS validation
						FROM 
							{$nomTypeRapport}
						WHERE 
		...
		$query .= " {$queryStr}
				...
				WHERE 
					dossier.client_dossier = {$id_client}
					AND dossier.type_dossier = 2
					AND dossier_echantillon_analyse = dossier.id_dossier
					AND date_reception_dossier BETWEEN '{$date_debut}' AND '{$date_fin}'
				
				ORDER BY 
					dossier.id_dossier DESC,
					echantillon_analyse.id_echantillon_analyse DESC";
		$db->setQuery($query);
		// echo $query;
	...
	function detailsDossierMulti($id_client, $date_debut, $date_fin, $sharingKey) {
		$db = &GBMNet::getDBOGBMNet();
		$typeAnalyse = CEAPICWorldModelRapport::getTypeRapportByTypeAndMulti(CEAPICWorldModelRapport::TYPE_RAPPORT_ANALYSE, 1);
	...
	function detailDossierMono($id_client, $date_debut, $date_fin, $sharingKey) {
		...
		$db = &GBMNet::getDBOGBMNet();
		$typeAnalyse = CEAPICWorldModelRapport::getTypeRapportByTypeAndMulti(CEAPICWorldModelRapport::TYPE_RAPPORT_ANALYSE, 0);
	...
	// ======================================================================================================
	// Liste stratégie chantier d'un chantier
	// ======================================================================================================
	...
	// ======================================================================================================
	// Liste révision stratégie chantier d'un echantillon
	// ======================================================================================================

	function ListeRevisionStrategieChantier($id_echantillon) {
		$db = &GBMNet::getDBOGBMNet();
		$query = "SELECT 
					id_strategie_chantier as id_strategie, 
				...
				SELECT 
					id as id_strategie, 
					revision as revision_strategie_chantier,
					2 as type_strategie
				FROM 
					strategie_chantier_v1
				WHERE 
					validation = 1
					AND id_echantillon = {$id_echantillon}
	...
	function getRapportsMateriaux($id_echantillon) {
		$db = &GBMNet::getDBOGBMNet();
		$query = "SELECT 
					rapport_materiau.*
				FROM 
					rapport_materiau
				WHERE 
					echantillon_rapport_materiau = {$id_echantillon}
				ORDER BY 
					revision_rapport_materiau DESC";
		$db->setQuery($query);
	...
	// ======================================================================================================
	// Liste les rapports du chantier
	// ======================================================================================================
	...
	// ======================================================================================================
	// Affiche les informations de l'échantillon et du rapport
	// ======================================================================================================
	...
	function ChercheRapportMateriauMulti($id_analyse) {
		$db = &GBMNet::getDBOGBMNet();
		$query = "Select *
				from analyse_materiau_multi, rapport_materiau_multi
				where analyse_materiau_multi = " . $id_analyse . "
				and id_rapport_materiau_multi = rapport_materiau_multi
				and validation_rapport_materiau_multi = 1
				
				order by revision_rapport_materiau_multi DESC";
		$db->setQuery($query);
	...
	// ======================================================================================================
	// Liste toutes les dernières révisions de rapport d'un chantier 
	// ======================================================================================================
