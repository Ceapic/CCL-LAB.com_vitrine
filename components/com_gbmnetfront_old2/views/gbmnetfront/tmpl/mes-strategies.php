...
$url = $model->GetRemoteToken() . "&task=ListeStrategieClient&id_client=" . $id_client . "&token=" . $token . "&format=raw";
...
<script language="javascript" type="text/javascript">
	...
	function AfficheStrategie(object) {
		var page = "<?php echo JURI::root(); ?>index.php?option=com_gbmnetfront&task=AfficheStrategieFront&format=raw&ref=" + jQuery(object).parent().find('.strategie_chantier option:selected').html().replace(" ", "-") + "&type_strategie=" + jQuery(object).parent().find('.typeStrategie').val() + "&id_strategie=" + jQuery(object).parent().find('.strategie_chantier').val() + "&id_client=<?php echo $id_client; ?>&type_client=<?php echo $type_client; ?>&tokenclient=<?php echo $token_client; ?>";
		// alert(page);
