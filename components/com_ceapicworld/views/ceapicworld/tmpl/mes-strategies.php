	...
	<table id="table_strategie" class="table table-striped table-bordered table-hover">
		...
		<tbody>
			...
			for ($i = 0; $i <= (count($Strategies)-1); $i++){
				...
				echo "</select><button style=\"margin-left: 5px;\" class=\"ceapicworld_button_radius ceapicworld_button_blue ceapicworld_button_shadow\" onClick=\"AfficheStrategie(this,".$oneStrategie->type_strategie.")\"><i class=\"fa fa-download\"></i></button></td></tr>";
...
<script language="javascript" type="text/javascript">
	...
	function AfficheStrategie(object,typeStrategie){
		var page = "<?php echo JURI::root(); ?>index.php?option=com_ceapicworld&task=AfficheStrategie&format=raw&ref="+ jQuery(object).parent().find('.strategie_chantier option:selected').html().replace(" ", "-")+"&id_strategie="+ jQuery(object).parent().find('.strategie_chantier').val()+"&type_strategie="+typeStrategie+"&id_client=<?php echo $id_client; ?>&type_client=<?php echo $type_client; ?>&tokenclient=<?php echo $token_client; ?>";
		// alert(page);
