...
<button class="ceapicworld_button_radius ceapicworld_button_blue ceapicworld_button_shadow" onClick="Extraction()"><i class="fa fa-download"></i> Extraction</button>
...
<script language="javascript" type="text/javascript">
	...
	jQuery(document).ready(function(){
		...
		// Select menu
		// =============================================
	jQuery('#chantier_extraction').select2({ width: '500px'});
	...
	function Extraction(){
		...
		if(diff/365.25 > 1){
			...
			var page = "<?php echo JURI::root(); ?>index.php?option=com_ceapicworld&task=Extraction&format=raw&id_chantier="+jQuery('#chantier_extraction').val()+"&debut_extraction="+jQuery('#debut_extraction').val()+"&fin_extraction="+jQuery('#fin_extraction').val()+"&tokenchantier="+jQuery('#chantier_extraction').find('option:selected').attr('tokenchantier')+"&id_client=<?php echo $id_client; ?>&type_client=<?php echo $type_client; ?>&tokenclient=<?php echo $token_client; ?>";
