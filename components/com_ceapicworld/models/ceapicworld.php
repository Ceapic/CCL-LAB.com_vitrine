...
class CEAPICWorldModelceapicworld extends JModelLegacy {
	...
	function CheckRemoteToken($token){
		...
		return file_get_contents($remotetoken."/index.php?option=com_ceapicworld&task=CheckTokenCEAPIC&token=".$token."&format=raw");
	}
	...
	function NouveauUtilisateur($login, $pwd, $client, $type_client, $idLaboratoire){	
		$data =new stdClass();
		$data->id = null;
		$data->name = $login;
		$data->username = $login;
    $data->id_laboratoire = $idLaboratoire;
		$data->email = $login."@ceapic.fr";
	...
	function ListeUtilisateurClient($client, $type_client, $idLaboratoire){
		$query = "Select *
				from #__users
				where id_client = '".$client."'
        and id_laboratoire = '".$idLaboratoire."'
