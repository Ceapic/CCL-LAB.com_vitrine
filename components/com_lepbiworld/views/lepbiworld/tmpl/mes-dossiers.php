...
/*
$url = $model->GetRemoteToken() . "/index.php?option=com_lepbiworld&task=ListeEchantillonAnalyse&date_debut=" . $date_debut . "&date_fin=" . $date_fin . "&id_client=" . $id_client . "&token=" . $token . "&format=raw";
$Echantillons = json_decode(file_get_contents($url));
*/

$urlDossier = $model->GetRemoteToken() . "/index.php?option=com_lepbiworld&task=ListeDossierExterne&date_debut=" . $date_debut . "&date_fin=" . $date_fin . "&id_client=" . $id_client . "&token=" . $token . "&format=raw";
// echo $urlDossier;
...
<div id="div_liste_echantillon" class="panel panel-primary">
	<div class="panel-heading">
		<h3 class="panel-title">Liste des Dossiers</h3>
	</div>
	...
	<!--<div class="row">
			<div class="col-sm" style="text-align: right;">
				...
				<button type="button" class="lepbiworld_button_radius lepbiworld_button_green lepbiworld_button_shadow" style="margin-left:10px; margin-bottom:10px;" onClick="ExtractionAnalyseDate('META')"><i class="fa fa-file-excel-o"></i> Export excel</button>
			</div>
		</div>
		...
		<div class="row">
			<div class="col-sm"  style="text-align: right;">
				...
				<button type="button" class="lepbiworld_button_radius lepbiworld_button_green lepbiworld_button_shadow" style="margin-left:10px; margin-bottom:10px;" onClick="ExtractionAnalyseDate('MAT')"><i class="fa fa-file-excel-o"></i> Export excel</button>
			</div>
		</div>-->
	<div>
		...
		<!-- au <input type="text" class="date" name="date_fin_meta" id="date_fin_meta" value="<?php echo $date_fin_meta ?>" style="width: 80px; text-align: center;" readonly /> -->
		<!-- <button type="button" class="lepbiworld_button_radius lepbiworld_button_green lepbiworld_button_shadow" style="margin-left:10px; margin-bottom:10px;" onClick="ExtractionAnalyseDate('META')"><i class="fa fa-file-excel-o"></i> Export excel META</button>				 -->
		<!-- </div> -->
		<!-- </div> -->
		...
		<!-- au <input type="text" class="date" name="date_fin_materiau" id="date_fin_materiau" value="<?php echo $date_fin_materiau ?>" style="width: 80px; text-align: center;" readonly /> -->
		<!-- <button type="button" class="lepbiworld_button_radius lepbiworld_button_green lepbiworld_button_shadow" style="margin-left:10px; margin-bottom:10px;" onClick="ExtractionAnalyseDate('MAT')"><i class="fa fa-file-excel-o"></i> Export excel Matériau</button> -->
		<!-- </div> -->
		<!-- </div> -->
	</div>
	...
	<form action="#" method="post" name="myForm">
		<div class="row">
			<div class="col-sm" style="text-align: center;">
				...
				<button type="button" class="lepbiworld_button_radius lepbiworld_button_blue lepbiworld_button_shadow" style="margin-left:10px; margin-bottom:10px;" onClick="Refresh()"><i class="fa fa-refresh"></i> Actualiser </button>
				<button type="button" class="lepbiworld_button_radius lepbiworld_button_green lepbiworld_button_shadow" style="margin-left:10px; margin-bottom:10px;" onClick="ExtractionAnalyseDate('META')"><i class="fa fa-file-excel-o"></i> Exportation Excel META</button>
				<button type="button" class="lepbiworld_button_radius lepbiworld_button_green lepbiworld_button_shadow" style="margin-left:10px; margin-bottom:10px;" onClick="ExtractionAnalyseDate('MAT')"><i class="fa fa-file-excel-o"></i> Exportation Excel Matériau</button>
			</div>
		</div>
	</form>
	...
	<table id="table_analyse" class="table table-striped table-bordered table-hover">
		<thead>
			<tr>
				...
				<td linkcolumn="2">Référence Lepbi</td>
			...
			<tr>
				...
				<td linkcolumn="2" class="filter" sizeinput="200">Ref Lepbi</td>
		...
		<tbody>
			...
			foreach ($dossiers as $dossier) {
				if ($dossier->nb_ech > 0) {
					...
					// $adresse= "window.open(index.php?option=com_lepbiworld&task=ListeEchantillonAnalyse&date_debut=" . $date_debut . "&date_fin=" . $date_fin . "&id_dossier=" . $dossier->id_dossier . "&token=" . $token . "&format=raw', '_blank')";
					// onClick="'.$adresse.'">

					$btnExoMulti = "";
					if ($dossier->is_multi == "true") {
						$btnExoMulti = '<button type="button" 
							class="show_analyse lepbiworld_button_radius lepbiworld_button_blue lepbiworld_button_shadow" 
...
<script language="javascript" type="text/javascript">
	jQuery('#div_liste_echantillon').on('click', '.show_analyse', function() {
		...
		var page = "<?php echo JURI::root(); ?>index.php?option=com_lepbiworld&task=AfficheAnalyseEchantillon&format=raw&ref=" + ref_echantillon + "&typeAnalyse=" + typeAnalyse + "&id=" + jQuery(this).attr("linkechantillon") + "&tokenechantillon=" + jQuery(this).attr("hash") + "&id_client=<?php echo $id_client; ?>&type_client=<?php echo $type_client; ?>&tokenclient=<?php echo $token_client; ?>";
		...
		function Data2Table(Trow) {
			...
			if (echantillons) {
				echantillons.forEach(ech => {
					...
					rows += "<td>";
					if (ech.has_mono == "1") {
						rows += '<button class="show_analyse lepbiworld_button_radius lepbiworld_button_blue lepbiworld_button_shadow" data-typeanalyse="mono" data-refechantillon="' + ech.ref_echantillon_analyse + '" linkechantillon="' + ech.id_echantillon_analyse + '" hash="' + hashEchantillon + '"><i class="fa fa-file-pdf-o"></i> </button>';
					}
					rows += "</td>";
	...
	function ExtractionAnalyseDate(type_extract) {
		...
		var page = "<?php echo JURI::root(); ?>index.php?option=com_lepbiworld&task=ExtractionAnalyseDate&date_debut=" + date_debut + "&date_fin=" + date_fin + "&type_extract=" + type_extract + "&id_client=<?php echo $id_client; ?>&type_client=<?php echo $type_client; ?>&tokenclient=<?php echo $token_client; ?>&format=raw";
