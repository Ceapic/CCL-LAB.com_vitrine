...
JHtml::script("components/com_lepbiworld/js/sheetJS/xlsx.full.min.js");
...
$remoteToken = $model->GetRemoteToken();

/*echo "<pre style='background:#ffc;padding:10px;border:2px solid #f90;'>";
echo "GetRemoteToken() retourne: " . htmlspecialchars($remoteToken) . "\n";
echo "</pre>";*/

$url = $remoteToken . "/index.php?option=com_lepbiworld&task=ListeChantier&id_client=" . $id_client . "&token=" . $token . "&format=raw";

//var_dump($url);
//die();

//$chantiersData = json_decode((file_get_contents($url)),true);
$chantiersData = json_decode(file_get_contents($url));
//var_dump($chantiersData);
//die();
...
<div id="tabs">
	...
	<div id="div_liste_chantier" class="panel panel-primary table-responsive-dt">
		...
		<table id="table_chantier" class="table table-striped table-bordered table-hover">
			<thead>
				...
				<tr>
					...
					<th class="filter">Adresse</th>
					<th class="filter">Ville</th>
					<th class="filter">Code postal</th>
					<th class="filter">Date mesure</th>
					<th></th>
				</tr>
			</thead>
			<tbody>
				<?php
				foreach ($chantiersData as $oneChantier) {
					...
					echo "<tr>
						<td onClick=\"ClickChantier(this, " . $oneChantier->id_chantier . ")\"><span style=\"white-space: nowrap;\">" . $icon_strat . " " . $oneChantier->nom_chantier . "</span></td>
						<td onClick=\"ClickChantier(this, " . $oneChantier->id_chantier . ")\">" . $oneChantier->adresse_chantier . "</td>
						<td onClick=\"ClickChantier(this, " . $oneChantier->id_chantier . ")\">" . $oneChantier->ville_chantier . "</td>
						<td onClick=\"ClickChantier(this, " . $oneChantier->id_chantier . ")\">" . $oneChantier->cp_chantier . "</td>
						<td onClick=\"ClickChantier(this, " . $oneChantier->id_chantier . ")\">" . $date_chantier . "</td>
						<td><i class=\"fa fa-file-excel-o\" onClick=\"Extraction(" . $oneChantier->id_chantier . ",'" . $token_chantier . "')\"></i></td>
						</tr>";
				}
				?>
			</tbody>
	...
	<div id="div_suivi_chantier" class="panel panel-primary">
		<div id="suivi_chantier" style="display:flex;justify-content: space-around;width: 100%;align-items:center;margin-top:10px;margin-bottom: 10px;">
			<div style="display:flex;align-items:center;width: 500px; justify-content: space-around;">
				<label>Stratégie: </label>
				<select size="1" id="sl_strategie_chantier" name="echantillon_strategie_chantier" onChange="ChangeStrategie()" style="height: auto;margin-bottom: unset;">
					<option value="0">Sélectionnez une stratégie</option>
				</select>

				<button class="lepbiworld_button_radius lepbiworld_button_blue lepbiworld_button_shadow" onClick="TelechargeStrategie()"><i class="fa fa-download"></i> Télécharger la stratégie</button>
			</div>
			<button class="lepbiworld_button_radius lepbiworld_button_green lepbiworld_button_shadow" onClick="Commander()"><i class="fa fa-calendar-check-o"></i> Planifier une intervention</button>
		</div>

		<div id="suivi_chantier_loader"></div>

		<div id="commande_chantier_loader" title="Planification Intervention"></div>
	</div>
	...
	<div id="div_liste_rapport" class="panel panel-primary">
		<div style="display: flex;">
			...
			<div style="width: 25%; height: 37px; float: right;">
				<button class="lepbiworld_button_radius lepbiworld_button_green lepbiworld_button_shadow" style="float: right;" onClick="DownloadAllRapport()"><i class="fa fa-download"></i> Télécharger tous les rapports</button>
			</div>
		</div>
...
<script language="javascript" type="text/javascript">
	jQuery('#suivi_chantier_loader').on('click', '.show_rapport_echantillon', function() {
		...
		var page = "<?php echo $remoteToken; ?>/index.php?option=com_lepbiworld&task=AfficheRapportEchantillon&format=raw&ref=" + jQuery(this).find(".href").html().replace(" ", "-") + "&id_echantillon=" + dataEch + "&tokenEchantillon=" + dataEchHash + "&id_mission=" + dataMission + "&tokenMission=" + dataMissionHash + "&id_client=<?= $id_client ?>&type_client=<?= $type_client ?>&tokenClient=<?= $token_client ?>";
		//console.log(page);
		window.open(page, '_blank');
	});

	jQuery('#liste_rapport_loader').on('click', '.show_rapport', function() {
		var page = "<?php echo $remoteToken; ?>/index.php?option=com_lepbiworld&task=AfficheRapport&format=raw&ref=" + jQuery(this).find(".href").html().replace(" ", "-") + "&id_echantillon=" + jQuery(this).attr("linkechantillon") + "&id_rapport=" + jQuery(this).attr("linkrapport") + "&tokenrapport=" + jQuery(this).attr("hash") + "&id_client=<?php echo $id_client; ?>&type_client=<?php echo $type_client; ?>&tokenclient=<?php echo $token_client; ?>";
		//console.log(page);
		window.open(page, '_blank');
	});
	...
	function DownloadAllRapport() {
		...
		var page = "<?php echo $remoteToken; ?>/index.php?option=com_lepbiworld&task=DownloadAllRapportChantier&format=raw&id_chantier=" + jQuery('#id_chantier').val() + "&ref_chantier=" + ref_chantier + "&id_client=<?php echo $id_client; ?>&type_client=<?php echo $type_client; ?>&tokenclient=<?php echo $token_client; ?>";
		window.open(page, '_blank');
	}
	...
	function ClickChantier(object, id_chantier) {
		...
		jQuery.ajax({
			//$remoteToken
			url: "<?php echo JURI::root(); ?>index.php?option=com_lepbiworld&task=ListeStrategieChantier&format=raw&id_chantier=" + id_chantier + "&id_client=<?php echo $id_client; ?>&type_client=<?php echo $type_client; ?>&tokenclient=<?php echo $token_client; ?>",
			dataType: 'JSON',
			// async: false, 
		...
		let temp = "<?php echo JURI::root(); ?>index.php?option=com_lepbiworld&task=ListeRapportChantier&format=raw&id_chantier=" + id_chantier + "&id_client=<?php echo $id_client; ?>&type_client=<?php echo $type_client; ?>&tokenclient=<?php echo $token_client; ?>";

		jQuery.ajax({
			url: "<?php echo JURI::root(); ?>index.php?option=com_lepbiworld&task=ListeRapportChantier&format=raw&id_chantier=" + id_chantier + "&id_client=<?php echo $id_client; ?>&type_client=<?php echo $type_client; ?>&tokenclient=<?php echo $token_client; ?>",
			// async: false, 
			success: function(data) {
	...
	function ChangeStrategie() {
		...
		let url = "<?php echo JURI::root(); ?>index.php?option=com_lepbiworld&task=SuiviStrategieLoader&id_strategie=" + idStrategie + "&id_echantillon=" + id_echantillon + "&id_chantier=" + id_chantier + "&ref_strategie=" + ref_strategie + "&format=raw&id_client=<?php echo $id_client; ?>&type_client=<?php echo $type_client; ?>&tokenclient=<?php echo $token_client; ?>";

		jQuery.ajax({
			url: "<?php echo JURI::root(); ?>index.php?option=com_lepbiworld&task=SuiviStrategieLoader&id_strategie=" + idStrategie + "&id_echantillon=" + id_echantillon + "&id_chantier=" + id_chantier + "&ref_strategie=" + ref_strategie + "&format=raw&id_client=<?php echo $id_client; ?>&type_client=<?php echo $type_client; ?>&tokenclient=<?php echo $token_client; ?>",
			success: function(data) {
				jQuery("#suivi_chantier_loader").empty();
				if (id_echantillon != 0) {
		...
		/*
		jQuery.ajax({
			url: "<?php echo JURI::root(); ?>index.php?option=com_lepbiworld&task=ListeRevisionStrategieChantier&format=raw&id_echantillon=" + id_echantillon + "&id_client=<?php echo $id_client; ?>&type_client=<?php echo $type_client; ?>&tokenclient=<?php echo $token_client; ?>",
			dataType: 'JSON',
			success: function(data) {
				result = data;
	...
	function TelechargeStrategie() {
		...
		var page = "<?php echo JURI::root(); ?>index.php?option=com_lepbiworld&task=AfficheStrategie&format=raw&ref=" + reference + "&id_strategie=" + idStrategie + "&type_strategie=" + typeStrategie + "&id_client=<?php echo $id_client; ?>&type_client=<?php echo $type_client; ?>&tokenclient=<?php echo $token_client; ?>";
		//depart
		window.open(page, '_blank');
	}
	...
	function Commander() {
		...
		var monurl = "<?php echo JURI::root(); ?>index.php?option=com_lepbiworld&task=Commander&format=raw&id_echantillon=" + id_echantillon + "&id_client=<?php echo $id_client; ?>&type_client=<?php echo $type_client; ?>&tokenclient=<?php echo $token_client; ?>";

		jQuery("#commande_chantier_loader").load(monurl, function() {
			var dialogHeight = Math.round(jQuery(window).height() * 0.98);
			jQuery(this).dialog({
				height: dialogHeight,
				width: '90%',
				modal: true,
	...
	function Extraction(id_chantier, tokenchantier) {
		var page = "<?php echo JURI::root(); ?>index.php?option=com_lepbiworld&task=Extraction&format=raw&id_chantier=" + id_chantier + "&debut_extraction=&fin_extraction=&tokenchantier=" + tokenchantier + "&id_client=<?php echo $id_client; ?>&type_client=<?php echo $type_client; ?>&tokenclient=<?php echo $token_client; ?>";
		window.location = page;
	}

	function BonCommandExcel(id_strategie, token_strategie) {
		var page = "<?php echo JURI::root(); ?>index.php?option=com_lepbiworld&task=BonCommande&format=raw&id_strategie=" + id_strategie + "&tokenstrategie=" + token_strategie + "&id_client=<?php echo $id_client; ?>&type_client=<?php echo $type_client; ?>&tokenclient=<?php echo $token_client; ?>";
		window.location = page;
	}
</script>