...
$url = $model->GetRemoteToken() . "/index.php?option=com_lepbiworld&task=ListeStrategieClient&id_client=" . $id_client . "&token=" . $token . "&format=raw";
		...
		<tbody>
			...
			for ($i = 0; $i <= (count($Strategies) - 1); $i++) {
				...
				if (($save_strategie <> $oneStrategie->ref_echantillon) && ($i <> 0))
					echo "</select><button style=\"margin-left: 5px;\" class=\"lepbiworld_button_radius lepbiworld_button_blue lepbiworld_button_shadow\" onClick=\"AfficheStrategie(this)\"><i class=\"fa fa-download\"></i></button></td></tr>";
...
<script language="javascript" type="text/javascript">
	...
	function AfficheStrategie(object) {
		var page = "<?php echo JURI::root(); ?>index.php?option=com_lepbiworld&task=AfficheStrategie&format=raw&ref=" + jQuery(object).parent().find('.strategie_chantier option:selected').html().replace(" ", "-") + "&id_strategie=" + jQuery(object).parent().find('.strategie_chantier').val() + "&id_client=<?php echo $id_client; ?>&type_client=<?php echo $type_client; ?>&tokenclient=<?php echo $token_client; ?>";
		// alert(page);
