...
<div id="tabs">
		...
		<table id="table_chantier" class="table table-striped table-bordered table-hover">
			...
			<tbody>
				...
				foreach ($Chantiers as $oneChantier){
					...
					echo "<tr onClick=\"ClickChantier(this, ".$oneChantier->id_chantier.")\"><td class=\"colonne1\"><span style=\"white-space: nowrap;\">".$icon_strat ." ". $oneChantier->nom_chantier."</span></td><td class=\"colonne2\">".$oneChantier->adresse_chantier."</td><td class=\"colonne3\">".$oneChantier->ville_chantier."</td><td class=\"colonne4\">".$oneChantier->cp_chantier."</td><td class=\"colonne5\">".$date_chantier."</td></tr>";
				}
				?>
			</tbody>
		</table>
	...
	<div id="div_suivi_chantier" class="panel panel-primary">
		...
		<div id="suivi_chantier">
			...
			<button class="lepbiworld_button_radius lepbiworld_button_green lepbiworld_button_shadow" style="float: right;" onClick="Commander()"><i class="fa fa-calendar-check-o"></i> Planifier une intervention</button>
	...
	<div id="div_liste_rapport" class="panel panel-primary">
		...
		<div id="liste_rapport_loader"></div>
	</div>
</div>
...
<script language="javascript" type="text/javascript">
	...
	jQuery('#suivi_chantier_loader').on('click', '.show_rapport_echantillon', function(){
		var page = "<?php echo JURI::root(); ?>index.php?option=com_lepbiworld&task=AfficheRapportEchantillon&format=raw&ref="+ jQuery(this).find(".href").html().replace(" ", "-")+"&id_echantillon="+ jQuery(this).attr("link")+"&tokenechantillon="+ jQuery(this).attr("hash")+"&id_client=<?php echo $id_client; ?>&type_client=<?php echo $type_client; ?>&tokenclient=<?php echo $token_client; ?>";
		window.open(page,'_blank' );
	});
	
	jQuery('#liste_rapport_loader').on('click', '.show_rapport', function(){
		var page = "<?php echo JURI::root(); ?>index.php?option=com_lepbiworld&task=AfficheRapport&format=raw&ref="+ jQuery(this).find(".href").html().replace(" ", "-")+"&id_echantillon="+ jQuery(this).attr("linkechantillon")+"&id_rapport="+ jQuery(this).attr("linkrapport")+"&tokenrapport="+ jQuery(this).attr("hash")+"&id_client=<?php echo $id_client; ?>&type_client=<?php echo $type_client; ?>&tokenclient=<?php echo $token_client; ?>";
		window.open(page,'_blank' );
	});
	...
	function ClickChantier(object, id_chantier) {
		...
		jQuery(object).addClass("selected_chantier");
		...
		jQuery('#chantier_selected').html(jQuery(object).find("td:eq(0)").html()+ " : " + jQuery(object).find("td:eq(1)").html()+ " " + jQuery(object).find("td:eq(2)").html()+ " " + jQuery(object).find("td:eq(3)").html());
	...
	function ChangeStrategie() {
		...
		jQuery.ajax({
			url: "<?php echo JURI::root(); ?>index.php?option=com_lepbiworld&task=SuiviStrategieLoader&id_echantillon="+id_echantillon+"&id_chantier="+id_chantier+"&ref_strategie="+ref_strategie+"&format=raw&id_client=<?php echo $id_client; ?>&type_client=<?php echo $type_client; ?>&tokenclient=<?php echo $token_client; ?>",
			success: function(data) {
	...
	function TelechargeStrategie(){
		var page = "<?php echo JURI::root(); ?>index.php?option=com_lepbiworld&task=AfficheStrategie&format=raw&ref="+ jQuery('#echantillon_strategie_chantier option:selected').html() +"-"+ jQuery('#revision_strategie_chantier option:selected').html() +"&id_strategie="+ jQuery('#revision_strategie_chantier').val() +"&id_client=<?php echo $id_client; ?>&type_client=<?php echo $type_client; ?>&tokenclient=<?php echo $token_client; ?>";
		window.open(page,'_blank' );
	}
	...
	// function(){
		// var data_reservation = jQuery('#div_message_commande').html();
		// jQuery.ajax({
			// url: "<?php echo JURI::root(); ?>index.php?option=com_lepbiworld&task=SendReservation&format=raw&id_client=<?php echo $id_client; ?>&type_client=<?php echo $type_client; ?>&tokenclient=<?php echo $token_client; ?>",
			// dataType: 'POST',
			// data: {
				// html: data_reservation
			// },
			// success: function(data) {
	...
	jQuery('button.extraction').on( 'click', function () {
		// var debut = jQuery("#de	but_extraction").val();
		...
		// }else{
			var page = "<?php echo JURI::root(); ?>index.php?option=com_lepbiworld&task=Extraction&format=raw&id_chantier="+jQuery('#chantier_extraction').val()+"&debut_extraction=&fin_extraction=&tokenchantier="+jQuery('#chantier_extraction').find('option:selected').attr('tokenchantier')+"&id_client=<?php echo $id_client; ?>&type_client=<?php echo $type_client; ?>&tokenclient=<?php echo $token_client; ?>";
