...
class LepbiWorldController extends JControllerLegacy {

	function CheckTokenLEPBI() {
		global $mainframe;
	...
	function ModifierUtilisateur() {
		...
		$checktoken = $model->CheckRemoteToken($token);
		if ($checktoken == "true") {
			$user = $model->ListeUtilisateurClient($client, $type_client);
	...
	function ListeStrategieChantier() {
		...
		if (md5($id_client . $type_client . $sharingkey) == $tokenclient) {
			...
			$url = $model->GetRemoteToken() . "/index.php?option=com_lepbiworld&task=ListeStrategieChantier&id_chantier=" . $id_chantier . "&id_client=" . $id_client . "&token=" . $token . "&format=raw";
      //var_dump($url);
	...
	function ListeRevisionStrategieChantier() {
		...
		if (md5($id_client . $type_client . $sharingkey) == $tokenclient) {
			$token = $model->CreateToken(bin2hex(openssl_random_pseudo_bytes(16)));
			echo file_get_contents($model->GetRemoteToken() . "/index.php?option=com_lepbiworld&task=ListeRevisionStrategieChantier&id_echantillon=" . $id_echantillon . "&id_client=" . $id_client . "&token=" . $token . "&format=raw");
		} else {
			die('Erreur de s&eacute;curit&eacute;');
		}
	}
	...
	function Commander() {
		...
		if (md5($id_client . $type_client . $sharingkey) == $tokenclient) {
			$token = $model->CreateToken(bin2hex(openssl_random_pseudo_bytes(16)));
			echo file_get_contents($model->GetRemoteToken() . "/index.php?option=com_lepbiworld&task=Commander&id_echantillon=" . $id_echantillon . "&id_client=" . $id_client . "&token=" . $token . "&format=raw");
		} else {
			die('Erreur de s&eacute;curit&eacute;');
		}
	}
	...
	function BonCommande() {
		...
		if ((md5($id_client . $type_client . $sharingkey) == $tokenclient) && (md5($id_strategie . $sharingkey) == $tokenstrategie)) {
			$token = $model->CreateToken(bin2hex(openssl_random_pseudo_bytes(16)));
			$url = $model->GetRemoteToken() . "/index.php?option=com_lepbiworld&task=BonCommande&id_strategie=" . $id_strategie . "&id_client=" . $id_client . "&token=" . $token . "&format=raw";
	...
	function ListeRapportChantier() {
		...
		if (md5($id_client . $type_client . $sharingkey) == $tokenclient) {
			$token = $model->CreateToken(bin2hex(openssl_random_pseudo_bytes(16)));
			echo file_get_contents($model->GetRemoteToken() . "/index.php?option=com_lepbiworld&task=ListeRapportChantier&id_chantier=" . $id_chantier . "&id_client=" . $id_client . "&token=" . $token . "&format=raw");
		} else {
			die('Erreur de s&eacute;curit&eacute;');
		}
	}
	...
	function SuiviStrategieLoader() {
		...
		if (md5($id_client . $type_client . $sharingkey) == $tokenclient) {
			$token = $model->CreateToken(bin2hex(openssl_random_pseudo_bytes(16)));
			// var_dump($model->GetRemoteToken() . "/index.php?option=com_lepbiworld&task=SuiviStrategieLoader&id_echantillon=" . $id_echantillon . "&ref_strategie=" . $ref_strategie . "&id_chantier=" . $id_chantier . "&id_client=" . $id_client . "&token=" . $token . "&format=raw");
			echo file_get_contents($model->GetRemoteToken() . "/index.php?option=com_lepbiworld&task=SuiviStrategieLoader&id_echantillon=" . $id_echantillon . "&id_strategie=" . $id_strategie . "&ref_strategie=" . $ref_strategie . "&id_chantier=" . $id_chantier . "&id_client=" . $id_client . "&token=" . $token . "&format=raw");
		} else {
			die('Erreur de s&eacute;curit&eacute;');
		}
	}
	...
	// 		$token = $model->CreateToken(bin2hex(openssl_random_pseudo_bytes(16)));
	// 		$url = $model->GetRemoteToken() . "/index.php?option=com_lepbiworld&task=AfficheRapportEchantillon&id_echantillon=" . $id_echantillon . "&id_client=" . $id_client . "&token=" . $token . "&format=raw";
	...
	function AfficheRapportEchantillon() {
		...
		if (
			...
			$token = $model->CreateToken(bin2hex(openssl_random_pseudo_bytes(16)));
			$url = $model->GetRemoteToken() . "/index.php?option=com_lepbiworld&task=AfficheRapportEchantillon&id_echantillon={$id_echantillon}&id_mission={$id_mission}&id_client={$id_client}&token={$token}&format=raw";
	...
	function AfficheRapport() {
		...
		if ((md5($id_client . $type_client . $sharingkey) == $tokenclient) && (md5($id_rapport . $sharingkey) == $tokenrapport)) {
			$token = $model->CreateToken(bin2hex(openssl_random_pseudo_bytes(16)));
			$url = $model->GetRemoteToken() . "/index.php?option=com_lepbiworld&task=AfficheRapport&id_echantillon=" . $id_echantillon . "&id_rapport=" . $id_rapport . "&id_client=" . $id_client . "&token=" . $token . "&format=raw";
	...
	function AfficheRapportFinal() {
		...
		if ((md5($id_client . $type_client . $sharingkey) == $tokenclient) && (md5($id_rapport . $sharingkey) == $tokenrapport)) {
			$token = $model->CreateToken(bin2hex(openssl_random_pseudo_bytes(16)));
			$url = $model->GetRemoteToken() . "/index.php?option=com_lepbiworld&task=AfficheRapportFinal&id_echantillon=" . $id_echantillon . "&id_rapport=" . $id_rapport . "&id_client=" . $id_client . "&token=" . $token . "&format=raw";
	...
	function AfficheStrategie() {
		...
		if (md5($id_client . $type_client . $sharingkey) == $tokenclient) {
			$token = $model->CreateToken(bin2hex(openssl_random_pseudo_bytes(16)));
			$url = $model->GetRemoteToken() . "/index.php?option=com_lepbiworld&task=AfficheStrategie&id_strategie=" . $id_strategie . "&type_strategie=" . $type_strategie . "&id_client=" . $id_client . "&token=" . $token . "&format=raw";
	...
	function AfficheAnalyseEchantillon() {
		...
		if ((md5($id_client . $type_client . $sharingkey) == $tokenclient) && (md5($id . $sharingkey) == $tokenechantillon)) {
			$token = $model->CreateToken(bin2hex(openssl_random_pseudo_bytes(16)));
			$url = $model->GetRemoteToken() . "/index.php?option=com_lepbiworld&task=AfficheAnalyseEchantillon&typeAnalyse=" . $typeAnalyse . "&id=" . $id . "&id_client=" . $id_client . "&token=" . $token . "&format=raw";
			//echo $url;
			//die();
	...
	function Extraction() {
		...
		if ((md5($id_client . $type_client . $sharingkey) == $tokenclient) && (md5($id_chantier . $type_client . $sharingkey) == $tokenchantier) && ($id_chantier != "0")) {
			$token = $model->CreateToken(bin2hex(openssl_random_pseudo_bytes(16)));
			$url = $model->GetRemoteToken() . "/index.php?option=com_lepbiworld&task=Extraction&id_chantier=" . $id_chantier . "&date_debut=" . $date_debut . "&date_fin=" . $date_fin . "&id_client=" . $id_client . "&token=" . $token . "&format=raw";
	...
	function ExtractionAnalyseDate() {
		...
		if (md5($id_client . $type_client . $sharingkey) == $tokenclient) {
			$token = $model->CreateToken(bin2hex(openssl_random_pseudo_bytes(16)));
			$url = $model->GetRemoteToken() . "/index.php?option=com_lepbiworld&task=ExtractionAnalyseDate&date_debut=" . $date_debut . "&date_fin=" . $date_fin . "&type_extract=" . $type_extract . "&id_client=" . $id_client . "&token=" . $token . "&format=raw";
			//echo $url;
	...
	function DownloadAllRapportChantier() {
		...
		if (md5($id_client . $type_client . $sharingkey) == $tokenclient) {
			$token = $model->CreateToken(bin2hex(openssl_random_pseudo_bytes(16)));
			$url = $model->GetRemoteToken() . "/index.php?option=com_lepbiworld&task=DownloadAllRapportChantier&id_chantier=" . $id_chantier . "&type_rapport=" . $type_client . "&id_client=" . $id_client . "&token=" . $token . "&format=raw";
	...
	function SendReservation() {
		...
		if (md5($id_client . $type_client . $sharingkey) == $tokenclient) {
			...
			/*if ($agence_chantier == "2") {
				$mail->Username = "ris.lyon@lepbi.fr";
				$mail->Password = "NpfAYidl8$";
				$mail->SetFrom("ris.lyon@lepbi.fr", "planning LEPBI");
			} else {*/
			$mail->Username = "exploitation@lepbi.fr";
			$mail->Password = "mlehbili14";
			$mail->SetFrom("exploitation@lepbi.fr", "Exploitation LEPBI");
			//}
			...
			/*if ($agence_chantier == 2) {
				$mail->AddAddress("ris.lyon@lepbi.fr");
				// $mail->AddAddress("lepbi.exploitation.lyon@gmail.com");
			} else {*/
			$mail->AddAddress("exploitation@lepbi.fr");
			// $mail->AddAddress("lepbi.exploitation@gmail.com");
			//}
