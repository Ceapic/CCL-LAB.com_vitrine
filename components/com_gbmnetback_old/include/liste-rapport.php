			...
			foreach ($Rapports as $i => $oneRapport) {
				...
				if ($Rapports[$i + 1]->id_echantillon <> $oneRapport->id_echantillon) {
					...
					if ($oneRapport2->type == "MEST") {
						...
						// var_dump($AfficheRapport);
						// var_dump($AfficheRapport[0]->volume_liquide_filtration_rapport_mest);
						// var_dump($AfficheRapport[0]->masse_filtre_pese_initiale_rapport_mest);
						// var_dump($AfficheRapport[0]->masse_filtre_pese_finale_rapport_mest);
						// die("mest");
						if (isset($AfficheRapport[0]->masse_filtre_pese_finale_rapport_mest) && isset($AfficheRapport[0]->masse_filtre_pese_initiale_rapport_mest) && isset($AfficheRapport[0]->volume_liquide_filtration_rapport_mest)) {
					...
					if ($oneRapport2->type == "META050") {
						...
						//$AfficheRapport not null anymore
						// var_dump($AfficheRapport);
						// die();
						if ($oneRapport2->analyse_echantillon == 2) {
					...
					if ($oneRapport2->type == "META269v3") {
						...
						// var_dump($AfficheRapport);
						// var_dump($oneRapport->id_echantillon);
						// var_dump($oneRapport2->analyse_echantillon);
						// die();
						if ($oneRapport2->analyse_echantillon == 2) {
							echo "test meta269v3";
							...
							if (count($Analyses) <> 0) {
								...
								//sa rentre pas ici
								echo "test";
								var_dump($oneAnalyse = $Analyses[0]);
								...
								echo "test2";
								var_dump($Prepa);
								var_dump($oneAnalyse->id_analyse_meta_v0);
								die();
								$lastPrepa = $Prepa[0];
								if (($lastPrepa->decision_anomalie_grille_analyse_meta_v0_prepa == "Echantillon inanalysable") || ($oneAnalyse->decision_anomalie_reception_analyse_meta_v0 == "Echantillon inanalysable")) {
									$SA = "/";
