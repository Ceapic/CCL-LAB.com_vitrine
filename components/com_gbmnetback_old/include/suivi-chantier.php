...
// $modelStrategie = $this->getModel('strategie'); // nom modelStrategie
// $modelSynthese = $this->getModel('synthese'); // nom modelStrategie
...
$return = $modelStrategie->AfficheLastStrategieChantier($id_echantillon);
$oneStrategie = $return[0];

// $ref_strategie = str_replace("VS-", "RS-", $oneStrategie->ref_echantillon);

// $return = $modelStrategie->AfficheEchantillon($oneStrategie->echantillon_strategie_chantier);
...
foreach ($modelStrategie->ListeEchantillonStrategieChantier($id_echantillon) as $oneEchantillon) {
	$i++;
...
?>
		...
		if ($oneEchantillon->ref_echantillon <> "0") {
			$resultat = "";
			$oneRapport = $modelStrategie->AfficheTypeRapport($oneEchantillon->id_echantillon);
			if ($oneRapport->type == "MEST") {
				$AfficheRapport = $modelSynthese->AfficheRapportEchantillonMEST($oneEchantillon->id_echantillon);
			...
			if ($oneRapport->type == "META1") {
				$AfficheRapport = $modelSynthese->AfficheRapportEchantillonMETA11($oneEchantillon->id_echantillon);
				if ($oneRapport->analyse_echantillon == 2) {
					...
					$Analyses = $modelSynthese->AfficheAnalyseEchantillonMETA1($oneEchantillon->id_echantillon, 1);
					...
					if ($oneAnalyse->nombre_fibre_analyse_meta_1 > 3) {
						...
						$resultat = "&lt;" . number_format($modelSynthese->concentration_maximum($oneAnalyse->nombre_carres_analyse_meta_1, $oneAnalyse->nombre_fibre_analyse_meta_1, $oneAnalyse->volume_analyse_meta_1, $oneAnalyse->fraction_filtre_analyse_meta_1, $oneAnalyse->diametre_filtre_analyse_meta_1, $oneAnalyse->incertitude_analyse_meta_1, $modelSynthese->loi_poisson_Ns($oneAnalyse->nombre_fibre_analyse_meta_1), $oneAnalyse->surface_filtration_analyse_meta_1, $oneAnalyse->surface_grille_analyse_meta_1, $ED, $EPC, $ELPC, $D, $CC, $ECC, $EMIC, $ELMIC), 1, ',', '') . "f/L";
					}
				}
			...
			if ($oneRapport->type == "META050") {
				$AfficheRapport = $modelSynthese->AfficheRapportEchantillonMETA050($oneEchantillon->id_echantillon);
				if ($oneRapport->analyse_echantillon == 2) {
					...
					$Analyses = $modelSynthese->AfficheAnalyseEchantillonMETAv0($oneEchantillon->id_echantillon, 1);
					if (count($Analyses) <> 0) {
						$oneAnalyse = $Analyses[0];
					...
					$Analyses = $modelSynthese->getAnalysesEchantillonMETAv1($oneEchantillon->id_echantillon, 1);
					if (count($Analyses) <> 0) {
						$oneAnalyse = $Analyses[0];
			...
			if ($oneRapport->type == "META269") {
				$AfficheRapport = $modelSynthese->AfficheRapportEchantillonMETA269($oneEchantillon->id_echantillon);
				if ($oneRapport->analyse_echantillon == 2) {
			...
			if ($oneRapport->type == "META269v2") {
				$AfficheRapport = $modelSynthese->AfficheRapportEchantillonMETA269v2($oneEchantillon->id_echantillon);
				if ($oneRapport->analyse_echantillon == 2) {
			...
			if ($oneRapport->type == "META269v3") {
				$AfficheRapport = $modelSynthese->AfficheRapportEchantillonMETA269v3($oneEchantillon->id_echantillon);
				if ($oneRapport->analyse_echantillon == 2) {
		...
		$PhaseZone = "";
		// foreach($modelStrategie->ListeSynthetiqueTableStrategieChantier($oneStrategie->id_strategie_chantier) as $onePhaseZone){
		foreach ($modelStrategie->ListeSynthetiqueStrategieChantier($oneStrategie->id_strategie_chantier) as $onePhaseZone) {
			$detect_phase_zone = 0;
