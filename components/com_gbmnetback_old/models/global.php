...
class ModelGlobal {
	...
	function addItem($table, $newData, $id_name = "", $debug = false) {
		JFactory::getDBOGBMNet()->insertObject($table, $newData, $id_name);
		$stackTrace = debug_backtrace();
		Log::saveLog($table, JFactory::getDBOGBMNet()->insertid(), "Ajout", $newData, $stackTrace);

		if (JFactory::getDBOGBMNet()->getErrorNum() && $debug) {
			throw new Exception('AJOUT ' . JFactory::getDBOGBMNet()->getErrorMsg());
			echo JFactory::getDBOGBMNet()->getErrorMsg();
			exit;
		}
		return JFactory::getDBOGBMNet()->insertid();
	}
	...
	function editItem($table, $newData, $id_name, $debug = 0, $priseEnCompteNull = false) {
		...
		$resultat = JFactory::getDBOGBMNet()->UpdateObject($table, $newData, $id_name, $priseEnCompteNull);

		if (JFactory::getDBOGBMNet()->getErrorNum() && $debug == 1) {
			throw new Exception('MODIFICATION ' . JFactory::getDBOGBMNet()->getErrorMsg());
			echo JFactory::getDBOGBMNet()->getErrorMsg();
			exit;
		}
		return $resultat;
	...
	static function getItem($table, $id) {
		...
		return JFactory::getDBOGBMNet()->loadObject();
	}
	...
	static function getObjectOld($table, $id, $class) {
		...
		return JFactory::getDBOGBMNet()->loadObject($class);
	}
	...
	static function getObject($table, $id, $class, $debug = 0) {
		...
		if (JFactory::getDBOGBMNet()->getErrorNum() && $debug == 1) {
			...
			throw new Exception('SELECTION ' . JFactory::getDBOGBMNet()->getErrorMsg());
			echo JFactory::getDBOGBMNet()->getErrorMsg();
			exit;
		}
		return JFactory::getDBOGBMNet()->loadObject($class);
	...
	function deleteItem($table, $condition, $debug = 0) {
		...
		JFactory::getDBOGBMNet()->execute();

		Log::saveLog($table, 0, "Suppression", $condition);

		if (JFactory::getDBOGBMNet()->getErrorNum() && $debug == 1) {
			throw new Exception('DELETE ' . JFactory::getDBOGBMNet()->getErrorMsg());
			echo JFactory::getDBOGBMNet()->getErrorMsg();
			exit;
		}
	}
	...
	function newRevRapportPrlv($id_echantillon, $description = "") { // Ca ne sert à rien voir avec Nico
		...
		foreach ($type_rapport as $oneTypeRapport) {
			...
			JFactory::getDBOGBMNet()->setQuery($query);
			$result = JFactory::getDBOGBMNet()->loadObjectList();
