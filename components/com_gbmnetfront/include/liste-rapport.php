...
<STYLE>
	...
	#table_rapport .colonne1, #table_rapport .colonne3{
		width: 10%;
	}
	...
	<table id="table_rapport" class="table table-striped table-bordered" style="width: 100%;">
		...
		foreach($Rapports as $i=>$oneRapport){
			...
			if($Rapports[$i+1]->id_echantillon <> $oneRapport->id_echantillon){
				...
				$resultat = "";
				$oneRapport2 = $model->AfficheTypeRapport($oneRapport->id_echantillon);
				if($oneRapport2->type == "MEST"){
					$AfficheRapport = $model2->AfficheRapportEchantillonMEST($oneRapport->id_echantillon);
				...
				if($oneRapport2->type == "META1"){
					$AfficheRapport = $model2->AfficheRapportEchantillonMETA11($oneRapport->id_echantillon);
					if ($oneRapport2->analyse_echantillon == 2){
						...
						$Analyses = $model2->AfficheAnalyseEchantillonMETA1($oneRapport->id_echantillon, 1);
						$oneAnalyse = $Analyses[0];
						if ($oneAnalyse->nombre_fibre_analyse_meta_1 > 3){
							$resultat = number_format($model2->concentration_normalisee($oneAnalyse->nombre_fibre_analyse_meta_1, $oneAnalyse->nombre_carres_analyse_meta_1, $oneAnalyse->surface_grille_analyse_meta_1, $oneAnalyse->surface_filtration_analyse_meta_1, $oneAnalyse->volume_analyse_meta_1, $oneAnalyse->fraction_filtre_analyse_meta_1), 2, ',', '') ." f/L";
						}else{
							$resultat = "&lt;".number_format($model2->concentration_maximum($oneAnalyse->nombre_carres_analyse_meta_1, $oneAnalyse->nombre_fibre_analyse_meta_1, $oneAnalyse->volume_analyse_meta_1, $oneAnalyse->fraction_filtre_analyse_meta_1, $oneAnalyse->diametre_filtre_analyse_meta_1, $oneAnalyse->incertitude_analyse_meta_1, $model2->loi_poisson_Ns($oneAnalyse->nombre_fibre_analyse_meta_1), $oneAnalyse->surface_filtration_analyse_meta_1, $oneAnalyse->surface_grille_analyse_meta_1, $ED, $EPC, $ELPC, $D, $CC, $ECC, $EMIC, $ELMIC), 2, ',', '') ." f/L";
						}
					}
				...
				if($oneRapport2->type == "META050"){
					$AfficheRapport = $model2->AfficheRapportEchantillonMETA050($oneRapport->id_echantillon);
					// var_dump($oneRapport2);
					// die();
					if ($oneRapport2->analyse_echantillon == 2){
						...
						$Analyses = $model2->AfficheAnalyseEchantillonMETAv0($oneRapport->id_echantillon, 1);
						if(count($Analyses) <> 0){
							$oneAnalyse = $Analyses[0];
				...
				if($oneRapport2->type == "META269"){
					$AfficheRapport = $model2->AfficheRapportEchantillonMETA269($oneRapport->id_echantillon);
					if ($oneRapport2->analyse_echantillon == 2){
				...
				if($oneRapport2->type == "META269v2"){
					$AfficheRapport = $model2->AfficheRapportEchantillonMETA269v2($oneRapport->id_echantillon);
					if ($oneRapport2->analyse_echantillon == 2){
				...
				if($oneRapport2->type == "META269v3"){
					// echo $oneRapport->ref_echantillon." : METAv0<br>";
					$AfficheRapport = $model2->AfficheRapportEchantillonMETA269v3($oneRapport->id_echantillon);
					if ($oneRapport2->analyse_echantillon == 2){
				...
				if($oneRapport->type == "LIXIVIATION"){
					$AfficheRapport = $model2->AfficheRapportEchantillonLixiviation($oneRapport->id_echantillon);
				...
				if($oneRapport->type == "ECAILLE_PEINTURE"){
					$AfficheRapport = $model2->AfficheRapportEchantillonEcaillePeinture($oneRapport->id_echantillon);
				...
				if($oneRapport->type == "AIR_PLOMB"){
					$AfficheRapport = $model2->AfficheRapportEchantillonAirPlomb($oneRapport->id_echantillon);
				...
				if($oneRapport->type == "LINGETTE_PLOMB"){
					$AfficheRapport = $model2->AfficheRapportEchantillonLingettePlomb($oneRapport->id_echantillon);
					$resultat = $AfficheRapport[0]->concentration_surfacique_rapport_lingette_plomb;
				}
				
				if($oneRapport->type == "ACR"){
					$AfficheRapport = $model2->AfficheRapportEchantillonACR($oneRapport->id_echantillon);
				...
				echo '
				<tr>
					<td class="colonne1"><p class="show_rapport" linkechantillon="'.$oneRapport->id_echantillon.'" linkrapport="'.$oneRapport->rapport.'" hash="'.$hash.'"><span class="href">'.$oneRapport->ref_echantillon.' v'.$oneRapport->revision.'</span></p></td>
