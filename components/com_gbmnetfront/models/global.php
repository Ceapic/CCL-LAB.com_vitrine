...
class ModelGlobal {
	...
	function addItem($table, $newData, $id_name = "", $debug = false) {
		$GLOBALS["DB"]->insertObject($table, $newData, $id_name);
		$stackTrace = debug_backtrace();
		Log::saveLog($table, $GLOBALS["DB"]->insertid(), "Ajout", $newData, $stackTrace);

		if ($GLOBALS["DB"]->getErrorNum() && $debug) {
			throw new Exception('AJOUT ' . $GLOBALS["DB"]->getErrorMsg());
	...
	function editItem($table, $newData, $id_name, $debug = 0, $priseEnCompteNull = false) {
		...
		$resultat = $GLOBALS["DB"]->UpdateObject($table, $newData, $id_name, $priseEnCompteNull);

		if ($GLOBALS["DB"]->getErrorNum() && $debug == 1) {
			throw new Exception('MODIFICATION ' . $GLOBALS["DB"]->getErrorMsg());
			echo $GLOBALS["DB"]->getErrorMsg();
			exit;
		}
		return $resultat;
	...
	static function getObjectOld($table, $id, $class) {
		...
		return $GLOBALS["DB"]->loadObject($class);
	}
	...
	static function getObject($table, $id, $class, $debug = 0) {
		...
		if ($GLOBALS["DB"]->getErrorNum() && $debug == 1) {
			...
			throw new Exception('SELECTION ' . $GLOBALS["DB"]->getErrorMsg());
			echo $GLOBALS["DB"]->getErrorMsg();
			exit;
		}
		return $GLOBALS["DB"]->loadObject($class);
	...
	function deleteItem($table, $condition, $debug = 0) {
		...
		if ($GLOBALS["DB"]->getErrorNum() && $debug == 1) {
			throw new Exception('DELETE ' . $GLOBALS["DB"]->getErrorMsg());
			echo $GLOBALS["DB"]->getErrorMsg();
			exit;
		}
	}
	...
	function newRevRapportPrlv($id_echantillon, $description = "") { // Ca ne sert à rien voir avec Nico
		...
		foreach ($type_rapport as $oneTypeRapport) {
			...
			$GLOBALS["DB"]->setQuery($query);
			$result = $GLOBALS["DB"]->loadObjectList();
