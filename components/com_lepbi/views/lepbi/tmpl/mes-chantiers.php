...
JHtml::script("components/com_lepbi/js/jquery/jquery-1.12.4.min.js");
JHtml::script("components/com_lepbi/js/jquery-ui-1.12.1/jquery-ui.min.js");
JHtml::script("components/com_lepbi/js/jquery-ui-1.12.1/jquery.ui.datepicker-fr.js");
...
JHtml::script("components/com_lepbi/js/sheetJS/xlsx.full.min.js");
JHtml::stylesheet("components/com_lepbi/js/jquery-ui-1.12.1/jquery-ui.min.css");
JHtml::stylesheet("components/com_lepbi/css/font-awesome/css/font-awesome.min.css");

JHtml::script('components/com_lepbi/js/jquery.dataTables.min.js');
JHtml::stylesheet('components/com_lepbi/css/jquery.dataTables.min.css');
...
$url = $model->GetRemoteToken() . "/index.php?option=com_lepbi&task=ListeChantier&id_client=" . $id_client . "&token=" . 2147483647 . "&format=raw";
...
<script language="javascript" type="text/javascript">
	jQuery('#suivi_chantier_loader').on('click', '.show_rapport_echantillon', function() {
		...
		var page = "<?php echo JURI::root(); ?>index.php?option=com_lepbi&task=AfficheRapportEchantillon&format=raw&ref=" + jQuery(this).find(".href").html().replace(" ", "-") + "&id_echantillon=" + dataEch + "&tokenEchantillon=" + dataEchHash + "&id_mission=" + dataMission + "&tokenMission=" + dataMissionHash + "&id_client=<?= $id_client ?>&type_client=<?= $type_client ?>&tokenClient=<?= $token_client ?>";
		console.log(page);
		window.open(page, '_blank');
	});

	jQuery('#liste_rapport_loader').on('click', '.show_rapport', function() {
		var page = "<?php echo JURI::root(); ?>index.php?option=com_lepbi&task=AfficheRapport&format=raw&ref=" + jQuery(this).find(".href").html().replace(" ", "-") + "&id_echantillon=" + jQuery(this).attr("linkechantillon") + "&id_rapport=" + jQuery(this).attr("linkrapport") + "&tokenrapport=" + jQuery(this).attr("hash") + "&id_client=<?php echo $id_client; ?>&type_client=<?php echo $type_client; ?>&tokenclient=<?php echo $token_client; ?>";
		console.log(page);
		window.open(page, '_blank');
	});
	...
	function DownloadAllRapport() {
		...
		var page = "<?php echo JURI::root(); ?>index.php?option=com_lepbi&task=DownloadAllRapportChantier&format=raw&id_chantier=" + jQuery('#id_chantier').val() + "&ref_chantier=" + ref_chantier + "&id_client=<?php echo $id_client; ?>&type_client=<?php echo $type_client; ?>&tokenclient=<?php echo $token_client; ?>";
		window.open(page, '_blank');
	}
	...
	function ClickChantier(object, id_chantier) {
		...
		jQuery.ajax({
			url: "<?php echo JURI::root(); ?>index.php?option=com_lepbi&task=ListeStrategieChantierFront&format=raw&id_chantier=" + id_chantier + "&id_client=<?php echo $id_client; ?>&type_client=<?php echo $type_client; ?>&tokenclient=<?php echo $token_client; ?>",
			dataType: 'JSON',
			// async: false, 
		...
		let temp = "<?php echo JURI::root(); ?>index.php?option=com_lepbi&task=ListeRapportChantierFront&format=raw&id_chantier=" + id_chantier + "&id_client=<?php echo $id_client; ?>&type_client=<?php echo $type_client; ?>&tokenclient=<?php echo $token_client; ?>";

		jQuery.ajax({
			url: "<?php echo JURI::root(); ?>index.php?option=com_lepbi&task=ListeRapportChantierFront&format=raw&id_chantier=" + id_chantier + "&id_client=<?php echo $id_client; ?>&type_client=<?php echo $type_client; ?>&tokenclient=<?php echo $token_client; ?>",
			// async: false, 
			success: function(data) {
	...
	function ChangeStrategie() {
		...
		let url = "<?php echo JURI::root(); ?>index.php?option=com_lepbi&task=SuiviStrategieLoaderFront&id_strategie=" + idStrategie + "&id_echantillon=" + id_echantillon + "&id_chantier=" + id_chantier + "&ref_strategie=" + ref_strategie + "&format=raw&id_client=<?php echo $id_client; ?>&type_client=<?php echo $type_client; ?>&tokenclient=<?php echo $token_client; ?>";

		jQuery.ajax({
			url: "<?php echo JURI::root(); ?>index.php?option=com_lepbi&task=SuiviStrategieLoaderFront&id_strategie=" + idStrategie + "&id_echantillon=" + id_echantillon + "&id_chantier=" + id_chantier + "&ref_strategie=" + ref_strategie + "&format=raw&id_client=<?php echo $id_client; ?>&type_client=<?php echo $type_client; ?>&tokenclient=<?php echo $token_client; ?>",
			success: function(data) {
				jQuery("#suivi_chantier_loader").empty();
				if (id_echantillon != 0) {
	...
	function TelechargeStrategie() {
		...
		var page = "<?php echo JURI::root(); ?>index.php?option=com_lepbi&task=AfficheStrategie&format=raw&ref=" + reference + "&id_strategie=" + idStrategie + "&type_strategie=" + typeStrategie + "&id_client=<?php echo $id_client; ?>&type_client=<?php echo $type_client; ?>&tokenclient=<?php echo $token_client; ?>";
		//depart
		window.open(page, '_blank');
	}
	...
	function Commander() {
		...
		var monurl = "<?php echo JURI::root(); ?>index.php?option=com_lepbi&task=Commander&format=raw&id_echantillon=" + id_echantillon + "&id_client=<?php echo $id_client; ?>&type_client=<?php echo $type_client; ?>&tokenclient=<?php echo $token_client; ?>";
	...
	function Extraction(id_chantier, tokenchantier) {
		var page = "<?php echo JURI::root(); ?>index.php?option=com_lepbi&task=Extraction&format=raw&id_chantier=" + id_chantier + "&debut_extraction=&fin_extraction=&tokenchantier=" + tokenchantier + "&id_client=<?php echo $id_client; ?>&type_client=<?php echo $type_client; ?>&tokenclient=<?php echo $token_client; ?>";
		window.location = page;
	}

	function BonCommandExcel(id_strategie, token_strategie) {
		var page = "<?php echo JURI::root(); ?>index.php?option=com_lepbi&task=BonCommande&format=raw&id_strategie=" + id_strategie + "&tokenstrategie=" + token_strategie + "&id_client=<?php echo $id_client; ?>&type_client=<?php echo $type_client; ?>&tokenclient=<?php echo $token_client; ?>";
		window.location = page;
	}
</script>