...
$urlDossier = $model->GetRemoteToken() . "&task=ListeDossierExterne&date_debut=" . $date_debut . "&date_fin=" . $date_fin . "&id_client=" . $id_client . "&token=" . $token . "&format=raw";
// echo $urlDossier;
	...
	<table id="table_analyse" class="table table-striped table-bordered table-hover">
		<thead>
			<tr>
				...
				<td linkcolumn="2">Référence Ceapic</td>
...
<script language="javascript" type="text/javascript">
	jQuery('#div_liste_echantillon').on('click', '.show_analyse', function() {
		...
		var page = "<?php echo JURI::root(); ?>index.php?option=com_gbmnetfront&task=AfficheAnalyseEchantillonFront&format=raw&ref=" + ref_echantillon + "&typeAnalyse=" + typeAnalyse + "&id=" + jQuery(this).attr("linkechantillon") + "&tokenechantillon=" + jQuery(this).attr("hash") + "&id_client=<?php echo $id_client; ?>&type_client=<?php echo $type_client; ?>&tokenclient=<?php echo $token_client; ?>";
	...
	function ExtractionAnalyseDate(type_extract) {
		...
		var page = "<?php echo JURI::root(); ?>index.php?option=com_gbmnetfront&task=ExtractionAnalyseDateFront&date_debut=" + date_debut + "&date_fin=" + date_fin + "&type_extract=" + type_extract + "&id_client=<?php echo $id_client; ?>&type_client=<?php echo $type_client; ?>&tokenclient=<?php echo $token_client; ?>&format=raw";
