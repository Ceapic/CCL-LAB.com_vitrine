...
die("test");
$stratV1 = $model0->getStrategieChantierV1FromEchantillon($id_echantillon);

$Strategie = null;
$Chantier = null;
$DynamicTable = null;
$Strategie = $model0->AfficheLastStrategieChantier($id_echantillon);
$Chantier = $model0->AfficheChantierEchantillon($id_echantillon);
$typeStrategie = 2;
...
$model0->ListeContactChantier($Chantier[0]->id_chantier);
	...
	<?php
		...
		foreach ($DynamicTable as $oneDynamicTable) {
			...
			if ($oneDynamicTable->TYPE_TABLE == 2) {
				...
				$Mesure = $model0->AfficheMesureStrategie($Rows[0]->type_mesure_zone_travail_multi_row);
		...
		<div>
			<table style=\"width: 98%;\" class=\"table_gbmnet\">
				...
				for ($count = 1, $finish = count($Rows); $count < $finish; $count++) {
					$Mesure = $model0->AfficheMesureStrategie($Rows[$count]->type_mesure_zone_travail_multi_row);
					$count_radio++;
					echo "
				...
				$RefProcessus = $model0->ListeZoneTravailMultiRefProcessus($oneDynamicTable->ID);
				...
				foreach ($model0->ListeObjectifStrategieRow($oneDynamicTable->ID) as $oneRow) {
					...
					$ZoneTravail = "";
					$separator = "";
					foreach ($model0->ListeZoneTravail($oneRow->zone_homogene_objectif_strategie_row) as $oneZoneTravail) {
						if ($ZoneTravail <> "") {
							$separator = ", ";
						}
		...
		foreach ($array_mesure_strategie as $id_contexte => $oneContexte) {
			...
			foreach ($oneContexte as $id_PI_ZC => $ZSEs) {
				...
				if ($PI_ZC != null) {
					...
					if (count($array_mesure_echantillon_non_commande_processus[$id_PI_ZC]) > 0) {
...
<div id="destinataire" title="Information nécessaire pour la réservation" class="hide">
	<fieldset>
		...
		<select id="identite_reservation">
			...
			<?php foreach ($model0->ListeContactChantier($Chantier[0]->id_chantier) as $oneContact) { ?>
	...
	<fieldset>
		...
		<?php
		$count_checkbox = 0;
		foreach ($model0->ListeContactChantier($Chantier[0]->id_chantier) as $oneContact) {
			$count_checkbox++;
		?>
	...
	<fieldset>
		...
		<?php
		$count_checkbox = 0;
		foreach ($model0->ListeContactSite($Chantier[0]->id_chantier) as $oneContact) {
			$count_checkbox++;
		?>
